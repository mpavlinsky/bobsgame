...
private static extern(C) int* AudioAttenuator_create();
private static extern(C) int* AudioAttenuator_destroy(int* aObjHandle);
private static extern(C) float AudioAttenuator_attenuate(int* aObjHandle, float aDistance, float aMinDistance, float aMaxDistance, float aRolloffFactor);
...
private static extern(C) int Modplug_loadMemEx(int* aObjHandle, ubyte* aMem, uint aLength, int aCopy, int aTakeOwnership);
private static extern(C) int Modplug_loadFile(int* aObjHandle, int* aFile);
private static extern(C) void Modplug_setVolume(int* aObjHandle, float aVolume);
...
private static extern(C) void Modplug_set3dAttenuator(int* aObjHandle, int* aAttenuator);
private static extern(C) void Modplug_setInaudibleBehavior(int* aObjHandle, int aMustTick, int aKill);
...
private static extern(C) void TedSid_set3dProcessing(int* aObjHandle, int aDo3dProcessing);
