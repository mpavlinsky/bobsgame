...
AudioAttenuator_destroy = soloud_dll.AudioAttenuator_destroy
AudioAttenuator_destroy.restype = None
AudioAttenuator_destroy.argtypes = [ctypes.c_void_p]

AudioAttenuator_attenuate = soloud_dll.AudioAttenuator_attenuate
AudioAttenuator_attenuate.restype = ctypes.c_float
AudioAttenuator_attenuate.argtypes = [ctypes.c_void_p, ctypes.c_float, ctypes.c_float, ctypes.c_float, ctypes.c_float]
...
Modplug_loadMem = soloud_dll.Modplug_loadMem
Modplug_loadMem.restype = ctypes.c_int
Modplug_loadMem.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_ubyte), ctypes.c_uint]

Modplug_loadMemEx = soloud_dll.Modplug_loadMemEx
Modplug_loadMemEx.restype = ctypes.c_int
Modplug_loadMemEx.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_ubyte), ctypes.c_uint, ctypes.c_int, ctypes.c_int]

Modplug_loadFile = soloud_dll.Modplug_loadFile
Modplug_loadFile.restype = ctypes.c_int
Modplug_loadFile.argtypes = [ctypes.c_void_p, ctypes.c_void_p]

Modplug_setVolume = soloud_dll.Modplug_setVolume
Modplug_setVolume.restype = None
Modplug_setVolume.argtypes = [ctypes.c_void_p, ctypes.c_float]
...
Modplug_set3dAttenuator = soloud_dll.Modplug_set3dAttenuator
Modplug_set3dAttenuator.restype = None
Modplug_set3dAttenuator.argtypes = [ctypes.c_void_p, ctypes.c_void_p]

Modplug_setInaudibleBehavior = soloud_dll.Modplug_setInaudibleBehavior
Modplug_setInaudibleBehavior.restype = None
Modplug_setInaudibleBehavior.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int]
...
TedSid_set3dProcessing = soloud_dll.TedSid_set3dProcessing
TedSid_set3dProcessing.restype = None
TedSid_set3dProcessing.argtypes = [ctypes.c_void_p, ctypes.c_int]
...
class AudioAttenuator(object):
	def __init__(self):
		self.objhandle = AudioAttenuator_create()
	def __enter__(self):
		return self
	def __exit__(self, eType, eValue, eTrace):
		AudioAttenuator_destroy(self.objhandle)
		self.objhandle = ctypes.c_void_p(0)
		return False
	def close(self):
		AudioAttenuator_destroy(self.objhandle)
	...
	def quit(self):
		AudioAttenuator_destroy(self.objhandle)
		self.objhandle = ctypes.c_void_p(0)
	def attenuate(self, aDistance, aMinDistance, aMaxDistance, aRolloffFactor):
		return AudioAttenuator_attenuate(self.objhandle, ctypes.c_float(aDistance), ctypes.c_float(aMinDistance), ctypes.c_float(aMaxDistance), ctypes.c_float(aRolloffFactor))
...
class WavStream(object):
	...
	def load_to_mem(self, aFilename):
		return WavStream_loadToMem(self.objhandle, ctypes.c_char_p(aFilename))
	def load_file(self, aFile):
...
class Modplug(object):
	...
	def load_mem(self, aMem, aLength, aCopy = false, aTakeOwnership = true):
		return Modplug_loadMemEx(self.objhandle, ctypes.POINTER(ctypes.c_ubyte)(aMem), ctypes.c_uint(aLength), ctypes.c_int(aCopy), ctypes.c_int(aTakeOwnership))
	def load_file(self, aFile):
		return Modplug_loadFile(self.objhandle, aFile.objhandle)
	def set_volume(self, aVolume):
		Modplug_setVolume(self.objhandle, ctypes.c_float(aVolume))
	def set_looping(self, aLoop):
	...
	def set_3d_attenuator(self, aAttenuator):
		Modplug_set3dAttenuator(self.objhandle, aAttenuator.objhandle)
	def set_inaudible_behavior(self, aMustTick, aKill):
		Modplug_setInaudibleBehavior(self.objhandle, ctypes.c_int(aMustTick), ctypes.c_int(aKill))
	def set_filter(self, aFilterId, aFilter):
...
class Monotone(object):
	...
	def quit(self):
		Monotone_destroy(self.objhandle)
		self.objhandle = ctypes.c_void_p(0)
	def set_params(self, aHardwareChannels, aWaveform = SQUARE):
		return Monotone_setParamsEx(self.objhandle, ctypes.c_int(aHardwareChannels), ctypes.c_int(aWaveform))
	def load(self, aFilename):
		return Monotone_load(self.objhandle, ctypes.c_char_p(aFilename))
	def load_mem(self, aMem, aLength, aCopy = false, aTakeOwnership = true):
...
class TedSid(object):
	...
	def quit(self):
		...
		self.objhandle = ctypes.c_void_p(0)
	def load(self, aFilename):
		return TedSid_load(self.objhandle, ctypes.c_char_p(aFilename))
	def load_to_mem(self, aFilename):
		return TedSid_loadToMem(self.objhandle, ctypes.c_char_p(aFilename))
	def load_mem(self, aMem, aLength, aCopy = false, aTakeOwnership = true):
	...
	def set_3d_processing(self, aDo3dProcessing):
		TedSid_set3dProcessing(self.objhandle, ctypes.c_int(aDo3dProcessing))
	def set_3d_listener_relative(self, aListenerRelative):
