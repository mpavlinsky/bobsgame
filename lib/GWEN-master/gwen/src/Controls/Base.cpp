...
bool Base::OnMouseWheeled( int iDelta )
{
		...
		if (m_Parent)
		{
			//std::cout << "base parent" << std::endl;
			return m_Parent->OnMouseWheeled(iDelta);
		}

		if (m_ActualParent)
		{
			//std::cout << "base actual" << std::endl;
...
void Base::SetToolTip( const TextObject & strText )
{
	...
	if (GetSkin()->GetRender()->MeasureText(tooltip->GetFont(), strText.GetUnicode()).x>GetCanvas()->Width())
	{
		//SetWidth(GetCanvas()->Width());
		tooltip->SetWidth(GetCanvas()->Width());
