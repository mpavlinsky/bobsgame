...
void Menu::Layout( Skin::Base* skin )
{
	...
	for ( Base::List::iterator it = m_Base->Children.begin(); it != m_Base->Children.end(); ++it )
	{
		Base* pChild = ( *it );
	...
	m_Base->SetSize( Width(), childrenHeight );

	int scrollHeight = m_Base->Height();
	if ( Y() + scrollHeight > GetCanvas()->Height() )
	{ scrollHeight = GetCanvas()->Height() - Y(); }
	SetSize(Width(), scrollHeight);
...
{
	MenuItem* pItem = new MenuItem( m_Base );
...
{
	MenuDivider* divider = new MenuDivider(m_Base);
