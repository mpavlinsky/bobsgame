...
namespace ToolTip
{
	...
	void RenderToolTip( Skin::Base* skin )
	{
		...
		Gwen::Rect rOffset = Gwen::Rect( MousePos.x - Bounds.w * 0.5f, (MousePos.y) - Bounds.h - 10 + 60, Bounds.w, Bounds.h );
		Gwen::Rect canvasBounds = g_ToolTip->GetCanvas()->GetBounds();
		if(rOffset.y + Bounds.h>canvasBounds.h-Bounds.h)rOffset.y = canvasBounds.h - Bounds.h - 60;
		rOffset = Utility::ClampRectToRect( rOffset, canvasBounds);
