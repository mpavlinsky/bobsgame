	...
	namespace Controls
	{
		namespace Layout
		{
			...
			class GWEN_EXPORT TableRow : public Base
			{
					...
					Color TextColor()
					{
						if (!m_Columns[0]) { return Gwen::Color(0,0,0,255); }
						return m_Columns[0]->TextColor();
					}
...
//						Base::List::iterator it = std::find(Children.begin(), Children.end(), pRow);
//						if (it != Children.end()) {
//							Children.erase(it);
//						}
						//GetCanvas()->DoThink();
