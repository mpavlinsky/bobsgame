...
vec3 star(vec2 position) {
	...
	vec3 color = mix(vec3(-0.74, -0.5, -1.0), vec3(0.0, -0.5, -1.0), (position.y + 1.0) * 0.25);
	color += (vec3(0.5, 0.65, 0.30) * 1.0 / distance(vec2(0.0), position) * 0.075);
	color += vec3(0.5, 0.25, 0.30) * min(1.0, blade *0.7) * (1.0 / distance(vec2(0.0, 0.0), position)*0.075);
	return color;
...
void main( void ) {
	...
	i = 5.0;
		...
		p = vec2(r, c*.5)*SCALE*(LAYERS/(i*i));
		uv = fract(p)*2.0-1.0;
		m = clamp((rand(floor(p))-DENSITY/i)*BRIGHTNESS, 0.0, 1.520);
		color +=  clamp(star(uv*0.5)*m*dist, 0.0, 1.0);
