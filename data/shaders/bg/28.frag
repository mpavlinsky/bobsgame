...
{
	q.x = fbm( p  +0.60*cos(time));
	q.y = fbm( p + vec2(1.0));
	
	r.x = fbm( p +1.0*q + vec2(1.7,9.2)+0.35*sin(time) );
	r.y = fbm( p+ 1.0*q + vec2(8.3,2.8)+0.426*sin(time));
	//r = cMul(q,q+0.1*time);
	return fbm(p +1.0*r + 0.2* cos(time));
}

const vec3 color1 = vec3(0.8,0.1,0.9);
const vec3 color2 = vec3(0.0,1.0,0.0);
const vec3 color3 = vec3(0.2,0,0.3);
